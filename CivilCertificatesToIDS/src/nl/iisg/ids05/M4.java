...
public class M4 {
     ...
     public void convert(EntityManager em){   	
		 ...
		 if(getM4w_ay() > 0){
			 ...
			 int[] a = Utils.range(100, 18, 1, 1, getMar_cy());		
			 Utils.addIndiv(em, getIdnr(), Id_I_WT, "MC M4", "BIRTH_DATE", null, "Declared", "Estimated [18/100]", 
    	 ...
    	 if(getM4w_ll() != null && getM4w_ll().trim().length() > 0){
    		 ...
    		 if(ce != null)
    			 //Utils.addIndivContextAndContext(null, ce, em, getIdnr(), Id_I_WT, "MC M4", "Address", "Declared", "Exact", getMar_cd(), getMar_cm(), getMar_cy());
 		 	     Utils.addIndivAndContext(null, ce, em, getIdnr(), Id_I_WT, "MC M4", "RESIDENCE_LOCATION", "Declared", "Exact", getMar_cd(), getMar_cm(), getMar_cy());
