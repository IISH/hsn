...
//		for (Entry<Integer, Integer> entry : personNumbers.entrySet()) {		
	...
	private static int add(int x, int y){
	   ...
	   // Get length of id_person for x and y
	   
	   int lx = id_person[personNumber[x]].length;
	   int ly = id_person[personNumber[y]].length;
	   ...
	   int pny = personNumber[y];
		...
		if(id_person[personNumber[x]] == onlySelf)			
			id_person[personNumber[x]] = new int[ly];
		else{
	...
	private static void initDB(Connection connection){		
		...
		try {
			...
			id_person = new int[highest_ID_Person][100]; 
