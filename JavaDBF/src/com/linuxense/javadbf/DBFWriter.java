					...
					switch( fieldArray[j].getDataType()) {
						...
						case 'D':
							if( t_values[j] != null) {
								...
								t_sb.append(String.format("%4d%02d%02d", calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), calendar.get(Calendar.DAY_OF_MONTH))); 
                                outStream.write( Utils.textPadding( t_sb.toString(), fieldArray[j].getFieldLength()));
...
//								System.out.println( t_values[j].toString());
								outStream.write(
									Utils.textPadding(
										(t_values[j]).toString(),fieldArray[j].getFieldLength(), Utils.ALIGN_RIGHT));
							}
							else {
